...
async function commit () {
  if (!commitMsg.value.trim()) { toast.show(t('git.messageRequired')); return }
  if (!status.value?.staged?.length) { toast.show(t('git.nothingStaged')); return }
  busy.value = true
  try { await api.gitCommit(dir(), commitMsg.value.trim(), host.value); commitMsg.value = ''; toast.show(t('git.commitCreated')); refresh() }
  catch (ex) { toast.show(ex.message) }
  busy.value = false
}
async function pull () { busy.value = true; try { const r = await api.gitPull(dir(), host.value); toast.show(t('git.pullResult', { msg: (r.output || '').trim().split('\n').pop() || t('git.ok') })); refresh() } catch (ex) { toast.show(t('git.pullResult', { msg: ex.message })) } busy.value = false }
async function push () { busy.value = true; try { const r = await api.gitPush(dir(), host.value); toast.show(t('git.pushResult', { msg: (r.output || '').trim().split('\n').pop() || t('git.ok') })); refresh() } catch (ex) { toast.show(t('git.pushResult', { msg: ex.message })) } busy.value = false }
...
<template>
  <WindowFrame :win-id="winId" body-class="flexcol">
    <div class="finder-bar">
      <input class="fpath logp" v-model="repoPath" :placeholder="t('git.repoPath')" spellcheck="false" @keydown.enter="refresh">
      ...
      <template v-if="status && status.isRepo">
        ...
        <button class="fbtn" :disabled="busy" @click="pull">{{ t('git.pull') }}</button>
        <button class="fbtn" :disabled="busy" @click="push">{{ t('git.push') }}</button>
      </template>
    </div>

    <div v-if="status && !status.isRepo" class="finder-empty">{{ t('git.notARepo') }}<br><span style="font-size:12px;opacity:.6">{{ status.message }}</span></div>
    <div v-else-if="!status" class="finder-empty" style="font-size:13px">{{ t('git.enterRepoPath') }}</div>

    <div v-else class="git-main">
      <div class="git-left">
        <div class="git-sec">{{ t('git.staged', { n: status.staged.length }) }} <button v-if="status.staged.length" class="g-mini" @click="unstage('')">{{ t('git.unstageAll') }}</button></div>
        ...
        <div class="git-sec">{{ t('git.modified', { n: status.unstaged.length }) }} <button v-if="status.unstaged.length" class="g-mini" @click="stageAll">{{ t('git.stageAll') }}</button></div>
        ...
        <div class="git-sec">{{ t('git.untracked', { n: status.untracked.length }) }}</div>
        ...
        <div class="git-commit">
          <textarea v-model="commitMsg" :placeholder="t('git.commitMessage')" rows="2"></textarea>
          <button class="fbtn primary" :disabled="busy || !status.staged.length" @click="commit">{{ t('git.commit', { n: status.staged.length }) }}</button>
        </div>
      </div>
      <div class="git-right">
        <div class="git-tabs">
          <button :class="{ on: tab === 'diff' }" @click="tab = 'diff'">{{ t('git.diff') }}</button>
          <button :class="{ on: tab === 'log' }" @click="tab = 'log'">{{ t('git.history') }}</button>
        </div>
        <div v-if="tab === 'diff'" class="gdiff">
          <div v-if="!sel" class="finder-empty" style="font-size:13px">{{ t('git.selectFile') }}</div>
          <div v-else-if="!diff" class="finder-empty" style="font-size:13px">{{ t('git.noDiff') }}</div>
        ...
        <div v-else class="gitlog">
          ...
          <div v-if="!log.length" class="finder-empty" style="font-size:12px">{{ t('git.noCommit') }}</div>
        </div>
      </div>
    </div>
  </WindowFrame>
